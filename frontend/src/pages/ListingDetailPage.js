...
const ListingDetailPage = () => {
  ...
  const adminTab = location.state?.tab || 'listings';
  ...
  const handleBooking = async (e) => {
    e.preventDefault();
    if (!user) { toast.error('Please login to book a shelter'); navigate('/login'); return; }
    if (user.role !== 'SEEKER') { toast.error('Only shelter seekers can book listings'); return; }
  ...
  const handleBack = () => {
    if (fromAdmin) {
      navigate('/admin', { state: { tab: adminTab } });
    } else {
      navigate('/listings');
    }
  };
    ...
    <div className="min-h-screen-header bg-[#f9fafb]">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        >
          ...
          {fromAdmin ? 'Back to Admin Panel' : 'Back to Listings'}
        </button>
        ...
        <div className="bg-white border border-[#e5e7eb] rounded-xl overflow-hidden shadow-[0_8px_32px_rgba(0,0,0,0.06)]">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 p-4">
            {listing.photos && listing.photos.length > 0 ? (
              listing.photos.map((photo, index) => (
                <img key={index} src={photo} alt={`${listing.title} ${index + 1}`} className="w-full h-64 object-cover rounded-lg" />
              ))
            ) : (
          ...
          <div className="p-8">
            <div className="flex items-start justify-between mb-6">
              <div>
                <h1 className="text-3xl font-bold text-[#111827] mb-2" style={{ fontFamily: 'Outfit, sans-serif' }} data-testid="listing-detail-title">
                  {listing.title}
                </h1>
            ...
            {/* Who owns this space - now opens modal */}
            <div className="border-t border-[#e5e7eb] pt-6 mb-6">
              <h2 className="text-xl font-semibold text-[#111827] mb-4">Who owns this space</h2>
              <button
                onClick={() => openProfile(listing.provider_id)}
                className="flex items-center gap-4 p-4 bg-[#f9fafb] rounded-lg hover:bg-[#f3f4f6] transition-all w-full text-left"
                data-testid="provider-profile-link"
              >
              ...
              <div className="border-t border-[#e5e7eb] pt-6 mb-6">
                ...
                {blockedDates.length > 0 && (
                  <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mb-4">
                    <p className="text-sm text-yellow-800">Some dates are unavailable. Blocked dates are greyed out in the calendar.</p>
                  </div>
                )}
                <form onSubmit={handleBooking} className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-[#111827] mb-2">Check-in Date</label>
                      <Popover open={checkInOpen} onOpenChange={setCheckInOpen}>
                        <PopoverTrigger asChild>
                          <button type="button" className="w-full flex items-center px-4 py-3 border border-[#e5e7eb] rounded-lg text-left focus:outline-none focus:ring-2 focus:ring-[#e51636]/30 focus:border-[#e51636] bg-white" data-testid="check-in-date-input">
                        ...
                        <PopoverContent className="w-auto p-0" align="start">
                          <Calendar mode="single" selected={checkInDate} onSelect={(date) => { setCheckInDate(date); setCheckInOpen(false); if (checkOutDate && date && checkOutDate <= date) setCheckOutDate(null); }} disabled={disabledDays} initialFocus classNames={{ day_disabled: "text-[#d1d5db] opacity-30 line-through cursor-not-allowed bg-red-50" }} />
                        </PopoverContent>
                      </Popover>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-[#111827] mb-2">Check-out Date</label>
                      <Popover open={checkOutOpen} onOpenChange={setCheckOutOpen}>
                        <PopoverTrigger asChild>
                          <button type="button" className="w-full flex items-center px-4 py-3 border border-[#e5e7eb] rounded-lg text-left focus:outline-none focus:ring-2 focus:ring-[#e51636]/30 focus:border-[#e51636] bg-white" data-testid="check-out-date-input">
                            <CalendarIcon className="h-5 w-5 text-[#9ca3af] mr-3" />
                        ...
                        <PopoverContent className="w-auto p-0" align="start">
                          <Calendar mode="single" selected={checkOutDate} onSelect={(date) => { setCheckOutDate(date); setCheckOutOpen(false); }} disabled={[...disabledDays, ...(checkInDate ? [{ before: checkInDate }] : [])]} initialFocus classNames={{ day_disabled: "text-[#d1d5db] opacity-30 line-through cursor-not-allowed bg-red-50" }} />
                        </PopoverContent>
                      </Popover>
                    </div>
                  </div>
