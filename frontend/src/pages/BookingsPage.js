...
const BookingsPage = () => {
  ...
  const [seekerTab, setSeekerTab] = useState('upcoming');
  ...
  // Seeker tab filtering
  const getFilteredBookings = () => {
    if (user.role !== 'SEEKER') return allBookings;
    if (seekerTab === 'upcoming') return allBookings.filter(b => b.status === 'ACCEPTED' && b.check_out_date >= today);
    if (seekerTab === 'past') return allBookings.filter(b => b.status === 'ACCEPTED' && b.check_out_date < today);
    if (seekerTab === 'pending') return allBookings.filter(b => b.status === 'PENDING');
    return allBookings;
  };
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div className="flex-1">
          ...
          <div className="space-y-2 text-sm text-[#4b5563]">
            ...
            {/* Show notes */}
            {booking.notes && (
              <div className="flex items-start bg-blue-50 p-3 rounded-lg mt-2">
                <FileText className="h-4 w-4 mr-2 text-blue-600 mt-0.5 flex-shrink-0" />
                <div><p className="text-blue-800 font-medium text-sm">Notes:</p><p className="text-blue-700 text-sm">{booking.notes}</p></div>
              </div>
            )}
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {user.role === 'SEEKER' && (
          <div className="flex gap-2 mb-6 flex-wrap">
            <button onClick={() => setSeekerTab('upcoming')}
              className={`flex items-center gap-2 px-5 py-2.5 rounded-lg font-medium transition-all text-sm ${seekerTab === 'upcoming' ? 'bg-[#e51636] text-white' : 'bg-white border border-[#e5e7eb] text-[#4b5563] hover:bg-[#f9fafb]'}`} data-testid="tab-upcoming">
              <CalendarCheck className="h-4 w-4" />Upcoming
            </button>
            <button onClick={() => setSeekerTab('past')}
              className={`flex items-center gap-2 px-5 py-2.5 rounded-lg font-medium transition-all text-sm ${seekerTab === 'past' ? 'bg-[#e51636] text-white' : 'bg-white border border-[#e5e7eb] text-[#4b5563] hover:bg-[#f9fafb]'}`} data-testid="tab-past">
              <History className="h-4 w-4" />Past Bookings
            </button>
            <button onClick={() => setSeekerTab('pending')}
              className={`flex items-center gap-2 px-5 py-2.5 rounded-lg font-medium transition-all text-sm ${seekerTab === 'pending' ? 'bg-[#e51636] text-white' : 'bg-white border border-[#e5e7eb] text-[#4b5563] hover:bg-[#f9fafb]'}`} data-testid="tab-pending">
        ...
        : bookings.length === 0 ? (
          <div className="bg-white border border-[#e5e7eb] rounded-xl p-12 text-center">
            ...
            <h3 className="text-xl font-semibold text-[#111827] mb-2">No bookings</h3>
